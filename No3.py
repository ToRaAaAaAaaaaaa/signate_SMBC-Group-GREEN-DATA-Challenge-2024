# No2空の変更 IndustrytypeとPrimaryNAICSはほぼ同じ変数のためIndustrytypeを取り除いた。
# 変数を増やしFIPScodeを新たに追加
import pandas as pd
import numpy as np
...
train['FIPScode'] = train['FIPScode'].fillna(train['FIPScode'].mode()[0])
train['FIPScode'] = np.log(train['FIPScode'])
...
test['FIPScode'] = test['FIPScode'].fillna(test['FIPScode'].mode()[0])
test['FIPScode'] = np.log(test['FIPScode'])
...
# 説明変数と予測変数の決定
features_one = train[['Latitude', 'Longitude', 'FIPScode', 'PrimaryNAICS', 'GHG_Direct_Emissions_10_in_metric_tons', 'GHG_Direct_Emissions_11_in_metric_tons', 'GHG_Direct_Emissions_12_in_metric_tons', 'GHG_Direct_Emissions_13_in_metric_tons']].values
...
# 「test」の説明変数の値を取得
test_features = test[['Latitude', 'Longitude', 'FIPScode', 'PrimaryNAICS', 'GHG_Direct_Emissions_10_in_metric_tons', 'GHG_Direct_Emissions_11_in_metric_tons', 'GHG_Direct_Emissions_12_in_metric_tons', 'GHG_Direct_Emissions_13_in_metric_tons']].values
